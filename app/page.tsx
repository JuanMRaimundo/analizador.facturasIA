...
export default function Home() {
  ...
  const handleProcessQueue = async () => {
    ...
    for (let i = 0; i < fileQueue.length; i++) {
      ...
      // SI SE ACABARON LOS CREDITOS EN MEDIO DEL PROCESO
      if (usageCount >= MAX_FREE_TRIES) break;

      // ACTUALIZAMOS EL CARTELITO VISUAL
      ...
      try {
        ...
        // DELAY PARA EVITAR ERROR 429 DE GOOGLE
    ...
    setLoading(false);
    setCurrentFile(""); // Limpiamos el cartel
    setFileQueue([]);   // Limpiamos la cola
  };
  ...
  return (
    <main className="flex min-h-screen flex-col items-center p-8 bg-gray-950 text-white">
      ...
      <div className="w-full max-w-4xl grid grid-cols-1 md:grid-cols-2 gap-8 mb-12">
        <div>
           <h3 className="text-xl font-semibold mb-4 text-blue-300">1. Cargar Documentos</h3>
        ...
        <div className="bg-gray-900 rounded-xl p-4 border border-gray-800 h-64 overflow-y-auto">
          <h3 className="text-xl font-semibold mb-4 text-purple-300 flex justify-between">
            ...
            <span className="text-sm bg-gray-800 px-2 py-1 rounded">{fileQueue.length} archivos</span>
          </h3>
          
          {fileQueue.length === 0 ? (
            <p className="text-gray-500 text-center mt-10">No hay archivos en cola</p>
          ) : (
            <ul className="space-y-2">
              {fileQueue.map((file, idx) => (
                <li key={idx} className="flex justify-between items-center bg-gray-800 p-2 rounded text-sm">
        ...
        <div className="flex flex-col items-center gap-4 mb-12">
          ...
          >
            {usageCount >= MAX_FREE_TRIES 
              ...
              : `🚀 Procesar Facturas (${MAX_FREE_TRIES - usageCount} intentos restantes)`
            }
          </button>
          
          <p className="text-gray-500 text-xs mt-2">
            Modo Demo Portfolio: <span className="text-blue-300 font-bold">{MAX_FREE_TRIES - usageCount}</span> usos gratuitos disponibles.
          </p>
        </div>
      )}

      {/* 3. AQUÍ ESTÁ EL CARTELITO DE CARGA RESTAURADO*/}
      {loading && (
        <div className="mt-4 mb-12 p-6 bg-blue-900/20 border border-blue-500/50 rounded-xl flex flex-col items-center gap-4 animate-pulse">
           <div className="w-12 h-12 border-4 border-blue-400 border-t-transparent rounded-full animate-spin"></div>
           <p className="text-xl text-blue-200 font-mono text-center">
             {currentFile || "Iniciando motores de IA..."}
           </p>
        </div>
      )}
      ...
      {invoices.length > 0 && (
        <div className="w-full max-w-6xl animate-in fade-in slide-in-from-bottom-10">
          <div className="flex justify-between items-end mb-6 border-b border-gray-700 pb-4">
            <h2 className="text-3xl font-bold text-white">Reporte Financiero</h2>
            <button 
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
            ...
            <div className="bg-gray-800 p-6 rounded-xl border border-gray-700">
              ...
              <p className="text-4xl font-bold text-purple-400">{Object.keys(vendorStats).length}</p>
            </div>
          </div>
              ...
              <tbody>
                {Object.keys(vendorStats).map((vendor) => (
                  <tr key={vendor} className="border-b hover:bg-gray-50">
                    ...
                    <td className="p-4 text-center">{vendorStats[vendor].count}</td>
                    <td className="p-4 text-right font-bold text-blue-700">
                      $ {vendorStats[vendor].total.toLocaleString('es-AR', { minimumFractionDigits: 2 })}
                    </td>
                  </tr>
                ))}
              </tbody>
